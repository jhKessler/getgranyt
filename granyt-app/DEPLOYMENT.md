...
- [Installation Paths](#installation-paths)
  - [1. Install Script (Recommended)](#1-install-script-recommended)
...
- [Environment Variables](#environment-variables)
  ...
  - [Custom Deployment (Container Variables)](#custom-deployment-container-variables)
...
1. **Download the compose file:**
   ...
   ```
2. **Configure your environment:** Create a `.env` file using the [Docker Compose Variables](#docker-compose-env-file).
...
For advanced users deploying to Kubernetes, ECS, or using an external database (RDS, Supabase). You must provide the [Custom Deployment Variables](#custom-deployment-container-variables) directly to the containers.

1. **Pull the images:** `ghcr.io/jhkessler/granyt-app:latest` and `ghcr.io/jhkessler/granyt-app:latest-migrations`.
2. **Configure Secrets:** Set all required variables in your orchestration platform.
3. **Run Migrations:** Ensure the migrations image runs to completion before the main app starts.

---

## Environment Variables

The way you set environment variables depends on your installation path.

### Docker Compose (.env file)

When using the [Official Docker Compose](#2-official-docker-compose) or the [Install Script](#1-install-script-recommended), you provide high-level variables. The Compose file then automatically "synthesizes" the internal values (like `DATABASE_URL`) for the containers.
...
*Note: The `DATABASE_URL` is automatically generated by Docker Compose using `POSTGRES_PASSWORD`.*

---

### Custom Deployment (Container Variables)

When deploying to **Kubernetes**, **Bare Docker**, or **Cloud Platforms**, you must pass the final variables directly to the container environment.
...
#### ⚠️ Important for Custom Deployments
Docker Compose handles the "glue" between the database and the app. In a custom setup:
1. **Migrations:** You MUST run the `latest-migrations` image once before the `latest` app starts.
2. **Database:** Granyt expects a PostgreSQL database. If using an external one, ensure `DATABASE_URL` includes the correct credentials and host.

---

### General Configuration (Both Methods)
...
> 📋 Make sure to configure all [Custom Deployment Variables](#custom-deployment-container-variables) in your Kubernetes secrets.
