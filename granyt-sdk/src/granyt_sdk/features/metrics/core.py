...
class DataFrameMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        null_counts = {col.name: col.null_count for col in self.columns if col.null_count is not None}
        if null_counts:
            schema["null_counts"] = null_counts
            
        # Add empty string counts if computed
        empty_counts = {col.name: col.empty_string_count for col in self.columns if col.empty_string_count is not None}
        if empty_counts:
...
def _get_adapter(df: Any) -> Optional[DataFrameAdapter]:
...
def _validate_custom_metrics(metrics: Optional[Dict[str, Any]], name: str) -> Optional[Dict[str, Union[int, float]]]:
    ...
    for key, value in metrics.items():
        if not isinstance(key, str):
            raise TypeError(f"{name} keys must be strings, got {type(key).__name__} for key '{key}'")
